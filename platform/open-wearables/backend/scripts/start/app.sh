...
uv run python scripts/data_migrations/relabel_oura_hrv_sdnn_to_rmssd.py || echo "Warning: Oura HRV relabel failed — will retry on next startup."
...
uv run python scripts/data_migrations/backfill_is_daily_total.py || echo "Warning: is_daily_total backfill failed — will retry on next startup."
